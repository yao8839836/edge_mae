...
class LinkPredictor(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers,
        ...
        #m = torch.nn.Bilinear(in_channels, in_channels, 32) 
        #self.lins.append(m)
        self.h = torch.nn.Linear(25, hidden_channels)
        self.t = torch.nn.Linear(26, hidden_channels)
        self.r = torch.nn.Linear(1, hidden_channels)
        ...
        self.conv_layer = torch.nn.Conv1d(3, 1, 3)  # kernel size x 3
        #self.conv2_bn = torch.nn.BatchNorm1d(1)
        self.dropout = torch.nn.Dropout(0.0)
        self.non_linearity = torch.nn.ReLU() # you should also tune with torch.tanh() or torch.nn.Tanh()
        self.fc_layer = torch.nn.Linear((hidden_channels - 3 + 1) * 1, 1, bias=False)

        self.lins.append(self.conv_layer)
        self.lins.append(self.fc_layer)
    ...
    def forward(self, x_i, x_j, x_ij):
        ...
        out_conv = out_conv.view(-1, (self.hidden_channels - 3 + 1) * 1)
...
def main():
    parser = argparse.ArgumentParser(description='CNN Link Prediction')
    ...
    parser.add_argument('--output_model_path', type=str, default='results/cnn_link.pt')
    ...
    predictor = LinkPredictor(80, args.hidden_channels, 1,
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            if top_5_back > best_top5:
                ...
                with open("results/scores_cnn_val", "w") as f:
